...
export const FloorBlueprint: React.FC<FloorBlueprintProps> = ({
  ...
  const renderTable = (id: string, customClass = '') => {
    ...
    const customer =
      customerMap[tbl.id] ||
      (tbl.cellId ? customerMap[tbl.cellId] : undefined) ||
      customerMap[tbl.name] ||
      customerMap[`B${tbl.id}`];
    const currentStatus =
      statusMap[tbl.id] ||
      (tbl.cellId ? statusMap[tbl.cellId] : undefined) ||
      (customer ? 'booked' : (tbl.status || 'empty'));
  ...
  const vip1Customer = customerMap['VIP1'] || customerMap['2A'] || customerMap['LẦU 2A'];
  const vip1Table = getTable('VIP1');
  const vip1Status =
    statusMap['VIP1'] ||
    statusMap['2A'] ||
    (vip1Customer ? 'booked' : (vip1Table?.status || 'empty'));
  const isVip1Selected = selectedTables.has('VIP1');
  const isVip1SearchMatched = matchingTableIds.has('VIP1') || matchingTableIds.has('2A');
  const isVip1Disabled = isTableDisabled ? isTableDisabled('VIP1', vip1Status) : false;
  const vip1Badge = customBadges['VIP1'] || customBadges['2A'];
  const vip1Colors = STATUS_COLORS[vip1Status] || STATUS_COLORS.empty;

  // Helper for VIP2 (Lầu 2B)
  const vip2Customer = customerMap['VIP2'] || customerMap['2B'] || customerMap['LẦU 2B'];
  const vip2Table = getTable('VIP2');
  const vip2Status =
    statusMap['VIP2'] ||
    statusMap['2B'] ||
    (vip2Customer ? 'booked' : (vip2Table?.status || 'empty'));
  const isVip2Selected = selectedTables.has('VIP2');
  const isVip2SearchMatched = matchingTableIds.has('VIP2') || matchingTableIds.has('2B');
  const isVip2Disabled = isTableDisabled ? isTableDisabled('VIP2', vip2Status) : false;
  const vip2Badge = customBadges['VIP2'] || customBadges['2B'];
  const vip2Colors = STATUS_COLORS[vip2Status] || STATUS_COLORS.empty;

  return (
    <div
      id="floor-blueprint-container"
      className="w-[444px] min-w-[444px] max-w-[444px] border-2 border-black bg-white shadow-2xl rounded-sm overflow-hidden text-black font-sans select-none mx-auto"
    >
      {/* 1. Header: ĐƯỜNG THOẠI NGỌC HẦU */}
      <div
        id="blueprint-banner-thoai-ngoc-hau"
        className="w-full bg-[#d0d7de] border-b border-black py-1 px-2 text-center font-black text-xs tracking-wider uppercase"
      >
      ...
      <div
        ...
        className="w-full bg-[#eeeeee] border-b border-black py-0.5 text-center font-bold text-xs"
      >
        Bãi xe
      </div>
      ...
      <div className="flex w-full items-stretch border-b border-black">
        ...
        <button
          ...
          className={`w-10 shrink-0 border-r border-black flex items-center justify-center relative overflow-hidden p-1 transition-all ${
        ...
        >
          ...
          <span
            className="font-bold text-[10px] tracking-tight text-center leading-tight px-0.5 block"
            style={{
              writingMode: 'vertical-rl',
              transform: 'rotate(180deg)',
            }}
          >
            {vip1Status !== 'empty' && vip1Status !== 'inactive' && vip1Customer
              ? `LẦU 2A: ${vip1Customer}`
              : 'LẦU 2A ( VIP1: 50kh )'}
          </span>
        </button>
        ...
        {/* Thin Gray Strip */}
        <div className="w-2 shrink-0 bg-[#9e9e9e] border-r border-black" />

        {/* B. Green Garden Zone (Khu vực Xanh - Bàn 51 đến 64) */}
        <div
          id="blueprint-green-garden-zone"
          className="w-[106px] shrink-0 bg-[#c8e6c9] border-r border-black p-1.5 flex flex-col justify-between"
        >
        ...
        <div
          ...
          className="flex-1 min-w-[276px] bg-[#fff9c4] p-1.5 flex flex-col justify-between gap-1.5"
        >
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-1 justify-end flex-1">
              <div className="w-8 shrink-0">{renderTable('47')}</div>
              <div className="w-8 shrink-0">{renderTable('46')}</div>
              <div className="w-8 shrink-0">{renderTable('45')}</div>
              <div className="w-8 shrink-0">{renderTable('44')}</div>
            </div>
          </div>

          {/* Row 1: [7, 6] --- LỐI ĐI THẲNG --- [5, 4, 3, 2, 1] */}
          <div className="flex items-center justify-between">
            <div className="flex gap-1 w-[68px] shrink-0 justify-start">
              <div className="w-8 shrink-0">{renderTable('7')}</div>
              <div className="w-8 shrink-0">{renderTable('6')}</div>
            </div>
            {/* Lối đi thẳng giữa Bàn 6 và Bàn 5 */}
            <div className="w-6 shrink-0 flex items-center justify-center pointer-events-none">
              <span className="text-[7px] font-black text-amber-700/40 uppercase tracking-widest -rotate-90 select-none">
                LỐI ĐI
              </span>
            </div>
            <div className="flex gap-1 justify-end flex-1">
              <div className="w-8 shrink-0">{renderTable('5')}</div>
              <div className="w-8 shrink-0">{renderTable('4')}</div>
              <div className="w-8 shrink-0">{renderTable('3')}</div>
              <div className="w-8 shrink-0">{renderTable('2')}</div>
              <div className="w-8 shrink-0">{renderTable('1')}</div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex gap-1 w-[68px] shrink-0 justify-start">
              <div className="w-8 shrink-0">{renderTable('8')}</div>
              <div className="w-8 shrink-0">{renderTable('9')}</div>
            </div>
            <div className="w-6 shrink-0 flex items-center justify-center pointer-events-none" />
            <div className="flex gap-1 justify-end flex-1">
              <div className="w-8 shrink-0">{renderTable('10')}</div>
              <div className="w-8 shrink-0">{renderTable('11')}</div>
              <div className="w-8 shrink-0">{renderTable('12')}</div>
              <div className="w-8 shrink-0">{renderTable('13')}</div>
              <div className="w-8 shrink-0">{renderTable('14')}</div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex gap-1 w-[68px] shrink-0 justify-start">
              <div className="w-8 shrink-0">{renderTable('21')}</div>
              <div className="w-8 shrink-0">{renderTable('20')}</div>
            </div>
            <div className="w-6 shrink-0 flex items-center justify-center pointer-events-none" />
            <div className="flex gap-1 justify-end flex-1">
              <div className="w-8 shrink-0">{renderTable('19')}</div>
              <div className="w-8 shrink-0">{renderTable('18')}</div>
              <div className="w-8 shrink-0">{renderTable('17')}</div>
              <div className="w-8 shrink-0">{renderTable('16')}</div>
              <div className="w-8 shrink-0">{renderTable('15')}</div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex gap-1 w-[68px] shrink-0 justify-start">
              <div className="w-8 shrink-0">{renderTable('22')}</div>
              <div className="w-8 shrink-0">{renderTable('23')}</div>
            </div>
            <div className="w-6 shrink-0 flex items-center justify-center pointer-events-none" />
            {/* SÂN KHẤU Box */}
            <div className="flex-1 h-10 bg-[#2b2b2b] border border-black rounded-[2px] flex items-center justify-center shadow-inner">
              <span className="text-[#ffd54f] font-black text-xs tracking-wider uppercase">
                SÂN KHẤU
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex gap-1 w-[68px] shrink-0 justify-start">
              <div className="w-8 shrink-0">{renderTable('30')}</div>
              <div className="w-8 shrink-0">{renderTable('29')}</div>
            </div>
            <div className="w-6 shrink-0 flex items-center justify-center pointer-events-none" />
            <div className="flex gap-1 justify-end flex-1">
              <div className="w-8 shrink-0">{renderTable('28')}</div>
              <div className="w-8 shrink-0">{renderTable('27')}</div>
              <div className="w-8 shrink-0">{renderTable('26')}</div>
              <div className="w-8 shrink-0">{renderTable('25')}</div>
              <div className="w-8 shrink-0">{renderTable('24')}</div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex gap-1 w-[68px] shrink-0 justify-start">
              <div className="w-8 shrink-0">{renderTable('31')}</div>
              <div className="w-8 shrink-0">{renderTable('32')}</div>
            </div>
            <div className="w-6 shrink-0 flex items-center justify-center pointer-events-none" />
            <div className="flex gap-1 justify-end flex-1">
              <div className="w-8 shrink-0">{renderTable('33')}</div>
              <div className="w-8 shrink-0">{renderTable('34')}</div>
              <div className="w-8 shrink-0">{renderTable('35')}</div>
              <div className="w-8 shrink-0">{renderTable('36')}</div>
              <div className="w-8 shrink-0">{renderTable('37')}</div>
            </div>
          </div>

          {/* Row 7: [ 43 ] --- LỐI ĐI THẲNG --- [42, 41, 40, 39, 38] */}
          <div className="flex items-center justify-between">
            <div className="w-[68px] shrink-0">{renderTable('43', 'w-full')}</div>
            <div className="w-6 shrink-0 flex items-center justify-center pointer-events-none" />
            <div className="flex gap-1 justify-end flex-1">
              <div className="w-8 shrink-0">{renderTable('42')}</div>
              <div className="w-8 shrink-0">{renderTable('41')}</div>
              <div className="w-8 shrink-0">{renderTable('40')}</div>
              <div className="w-8 shrink-0">{renderTable('39')}</div>
              <div className="w-8 shrink-0">{renderTable('38')}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="w-full flex flex-col">
        ...
        <div className="flex w-full border-b border-black">
          <div className="w-[148px] bg-[#f57c00] border-r border-black py-0.5 px-1 text-left font-black text-xs">
            WC TRỆT
          </div>
        ...
        <div className="w-full bg-[#b0bec5] border-b border-black py-0.5 px-2 font-bold text-[11px]">
          HÀNH LANG, CẦU THANG BỘ, WC LẦU 1
        </div>

        {/* Row C: VIP Rooms (VIP 70, 72, 74, 76 - 10kh) */}
        <div className="w-full bg-[#c8e6c9] border-b border-black p-1.5">
          <div className="grid grid-cols-4 gap-2 max-w-[95%] mx-auto">
        ...
        <button
          ...
          className={`w-full py-1 text-center font-black text-xs tracking-wide uppercase transition-all ${
        ...
        >
          ...
          LẦU 2B ( VIP2: 80kh )
          {vip2Status !== 'empty' && vip2Status !== 'inactive' && vip2Customer ? (
            <span className="ml-2 font-bold text-[11px] normal-case opacity-95">
              - {vip2Customer}
            </span>
          ) : null}
        </button>
